...
@utils.duel_message_handler(message_constants.MSG_CARD_HINT)
...
def card_hint(client, card, type, value):
    if type == 3: # race announcement
        ...
        utils.output(_("{spec} ({name}) selected {value}.").format(spec=card.get_name(), name=card.get_name(), value=', '.join(races)))
        ...
        utils.output(_("{spec} ({name}) selected {value}.").format(spec=card.get_name(), name=card.get_name(), value=', '.join(attributes)))
