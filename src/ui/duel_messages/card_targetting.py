...
def card_target(client, card, target, cancelled):
    if not cancelled:
        utils.output(_("{card} targets {target}.").format(card=card.get_name(), target=target.get_name()))
        utils.get_ui_stack().play_duel_sound_effect("aim")
    else:
        utils.output(_("{card} cancels targeting {target}.").format(card=card.get_name(), target=target.get_name()))
