...
@utils.duel_message_handler(message_constants.MSG_RECOVER)
...
def recover(client, player, amount):
    if player == client.what_player_am_i:
        ...
        utils.output(_("Your lp increased by %d, now %d") % (amount, new_lp))
        client.player.update_lifepoints(new_lp)
    else:
        new_lp = client.player.opponent_lifepoints + amount
        utils.output(_("Your opponents lp increased by %d, now %d") % (amount, new_lp))
