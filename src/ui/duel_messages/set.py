...
@utils.duel_message_handler(message_constants.MSG_SET)
...
def set(client, card):
    ...
    if controller == client.what_player_am_i:
        utils.output(_("You set %s (%s) in %s position.") % (location_information.to_human_readable(), card.get_name(), card.get_position()))
    else:
        utils.output(_("Your opponent sets %s (%s) in %s position.") % (location_information.to_human_readable(), card.get_name(), card.get_position()))
