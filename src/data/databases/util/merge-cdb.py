...
def main() -> int:
	...
	if len(filtered_filenames) == 0:
		print(_("No databases to be merged found in '{directory}'").format(directory=db_directory), file=sys.stderr)
		return 2
	...
	print(_("Merging the following into '{filename}' {status}").format(filename=merged_db_filename, status=_("(Overriden)") if overriden else ""))

	conn = sqlite3.connect(merged_db_filename)
	conn.execute(DB_SCHEMA_1)
	conn.execute(DB_SCHEMA_2)
	for filename in filtered_filenames:
		print(_("... '{filename}'").format(filename=filename))
